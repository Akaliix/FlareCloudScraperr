...
def get_flaresolverr_version() -> str:
    ...
    search_paths = [
        os.path.join(os.path.dirname(os.path.abspath(__file__)), os.pardir, 'package.json'),
    ...
    for package_path in search_paths:
        if os.path.isfile(package_path):
            with open(package_path, encoding='utf-8') as f:
                FLARESOLVERR_VERSION = json.loads(f.read()).get('version', 'local-dev')
                return FLARESOLVERR_VERSION

    FLARESOLVERR_VERSION = 'local-dev'
    return FLARESOLVERR_VERSION

...
class CloudScraper(Session):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Metadata used by our external factory / refresh logic
        self.flaresolverr_proxy: str = kwargs.pop('flaresolverr_proxy', "")
        self.flaresolverr_bridge_http: str = kwargs.pop('flaresolverr_bridge_http', "")
        self.flaresolverr_target: str = kwargs.pop('flaresolverr_target', "")
    ...
    def request(self, method, url, *args, **kwargs):
        ...
        # If Cloudflare/owner returns 403, attempt to refresh cookies via FlareSolverr and retry once.
        if getattr(response, 'status_code', None) == 403:
            try:
                refreshed = self._refresh_cookies_via_flaresolverr()
                if refreshed:
                    # retry the request once with updated cookies/UA
                    try:
                        response = self.decodeBrotli(
                            self.perform_request(method, url, *args, **kwargs)
                        )
                        # apply post-hook if present
                        if self.requestPostHook:
                            newResponse = self.requestPostHook(self, response)
                            if response != newResponse:
                                response = newResponse
                    except Exception:
                        # if retry fails, keep original 403
                        pass
            except Exception:
                pass
    ...
    # ------------------------------------------------------------------------------- #
    # Helper to refresh cookies using FlareSolverr when configured
    # ------------------------------------------------------------------------------- #

    def _refresh_cookies_via_flaresolverr(self):
        """Attempt to refresh cookies by calling FlareSolverr using stored bridge info.

        Expected attributes on the scraper (set by external factory):
        - flaresolverr_bridge_http : the HTTP proxy URL for FlareSolverr to use
        - flaresolverr_target : the target URL to request via FlareSolverr
        """
        try:
            proxy = getattr(self, "flaresolverr_bridge_http", None)
            target = getattr(self, "flaresolverr_target", None)
            if not proxy or not target:
                return False
            
            # Import here to avoid top-level dependency cycle
            client = FlareSolverrClient()
            jar, ua = client.get_cookies(proxy_url=proxy, target_url=target)
            if jar:
                self.cookies = jar
            if ua:
